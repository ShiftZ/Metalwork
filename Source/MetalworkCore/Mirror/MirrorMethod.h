...
#define MIRROR_METHOD_META(_Name_, _NativeName_, ...) \
	xproperty_##_Name_##_type(); \
	struct xproperty_##_Name_##_meta \
	{ \
		using Scope = Class::Type; \
		std::string_view Name() { using namespace std::literals; return #_Name_##sv; } \
		auto Func() { return &Class::Type::_NativeName_; } \
		MIRROR_FORCEDSPEC static void* Register() \
		{ \
			xproperty_##_Name_##_meta meta; \
			Class::Construct()->AddMethod(new Class::MethodType(&meta)); \
