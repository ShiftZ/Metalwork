...
Network::Network(span<RemotePlayer> targets, int player, stop_token st) : player(player), udp(io)
{
	...
	{
		auto connect = [&](RemotePlayer& target) -> awaitable<void>
		{
			tcp::socket socket(io);
			tcp::endpoint tcp_endpoint(make_address(target.ip), target.port.tcp);
...
Task<INetwork> Network::Connect(span<RemotePlayer> targets, int player)
{
	vector targets_copy(targets.begin(), targets.end());

	packaged_task task([=](stop_token st) mutable -> unique_ptr<INetwork>
		{ return make_unique<Network>(targets_copy, player, st); });

	future<unique_ptr<INetwork>> future = task.get_future();
